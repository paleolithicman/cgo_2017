...
def mfcc_features(filename):
  ...
  #mfccs = librosa.feature.mfcc(d, sr, n_mfcc=13, n_fft=int(frame_overlap_seconds*sr), hop_length=int(frame_length_seconds*sr))

  # energy (TODO: log?)
  energy = librosa.feature.rmse(d, n_fft=int(frame_overlap_seconds*sr), hop_length=int(frame_overlap_seconds*sr))
  #energy = librosa.feature.rmse(d, n_fft=int(frame_overlap_seconds*sr), hop_length=int(frame_length_seconds*sr))
  mfccs = np.delete(mfccs, mfccs.shape[1]-1, 1)

  mfccs[0] = energy 
# replace first MFCC with energy, per convention
